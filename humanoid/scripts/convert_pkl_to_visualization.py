...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert retargeted motion PKL files into the project's motion_visualization schema."
    )
    parser.add_argument("--input", required=True, help="Path to a source .pkl motion file")
    parser.add_argument("--output", required=True, help="Path to the normalized motion_visualization .npz")
    parser.add_argument(
        "--use_default_joint_order",
        action="store_true",
        help="Reorder the motion to the Unitree G1 joint order used by this repository.",
    )
...
def main():
    ...
    print(f"Saved visualization motion to {args.output}")
    print(
